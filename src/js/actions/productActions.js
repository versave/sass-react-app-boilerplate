...
export const deleteProduct = (id) => (dispatch, getState) => {
    axios.delete(proxy(`/products/${id}`), tokenConfig(getState))
        .then(res => dispatch({
            type: DELETE_PRODUCT,
            payload: id
        }))
...
export const addProduct = (product) => (dispatch, getState) => {
    axios.post(proxy('/products'), product, tokenConfig(getState))
        .then(res => {
            dispatch(setProductsLoading());
...
export const editProduct = (id, edits) => (dispatch, getState) => {
    axios.patch(proxy(`/products/${id}`), edits, tokenConfig(getState))
        .then(res => {
            dispatch(setProductsLoading());
            dispatch(callLoaded(false));
